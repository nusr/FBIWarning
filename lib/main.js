...
class BaseSpider {
  ...
  async innerRecursion(requestUrls, arrayIndex) {
...
class parseDetailPage extends BaseSpider {
  ...
  async recursionExecutive() {
    ...
    try {
      let requestUrls = []
      let repeatItems = []
      tableLinks.forEach(url => {
        let item = tableList[url]
        if (!item.title) {
          requestUrls.push(COMMON_CONFIG.baseUrl + url)
        } else {
          // 有种子链接的直接下载
          repeatItems.push(url)
        }
      })
      for (let url of repeatItems) {
        let item = tableList[url]
        let directory = parentDir + "/" + url.replace(/\//gi, "_")
        await this.downloadResult(directory, item.torrents, item.images)
      }
...
class ParseTableList extends BaseSpider {
  ...
  recursionExecutive() {
    ...
    let maxLink = COMMON_CONFIG.maxDetailLinks
    if (IS_ADMIN) {
      maxLink = categoryList[currentCategory].theme
    }

    if (
      tableLinks.length >= maxLink ||
      this.categoryIndex >= categoryLinks.length
    ) {
      this.getDetailData()
      return false
    }
  ...
  parseHtml($) {
    ...
    tableDom.each(function() {
      ...
      let endPage = ~~$("#main .pages")
        ...
        .split("/")
        .pop()
      categoryList[this.currentCategory].endPage = Math.max(
        categoryList[this.currentCategory].endPage,
        endPage
      )
      // 详情页面链接
      let link = $(this)
...
class ParseCategory extends BaseSpider {
  ...
  recursionExecutive(isAdmin) {
    try {
      this.requestPage(COMMON_CONFIG.baseUrl).then($ => {
        if ($) {
          this.parseHtml($, isAdmin)
        } else {
          this.requestFailure()
        }
      })
  ...
  parseHtml($, isAdmin) {
    ...
    this.updateCategoryList()
    this.requestSuccess()
    if (isAdmin === COMMON_CONFIG.admin) {
      parseAllCategory = true
      new ParseTableList().recursionExecutive()
    } else {
      this.selectCategory()
