      ...
      function innerRecursion(arrayIndex) {
        ...
        async.mapLimit(
          ...
          async url => {
            ...
            let seed = url.slice(COMMON_CONFIG.baseUrl.length, urlLen)
            // 已经爬取了，直接下载，加快文件下载速度
            // 第一次解析的结果保存了，下次可以直接使用
...
class ParseTableList extends BaseSpider {
  ...
  recursionExecutive() {
    ...
    let startPage = 1
    if (parseAllCategory) {
      startPage = ~~(tableLinks.length / COMMON_CONFIG.pageSize) || 1
    }
