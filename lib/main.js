...
class BaseSpider {
  ...
  filterIllegalPath(path) {
    return path.match(/[a-zA-Z0-9]/gi).join("");
  }
  /**
   * 下载文件
...
class ParseTableList extends BaseSpider {
  ...
  recursionExecutive() {
    ...
    this.jsonPath =
      ...
      "/" +
      this.filterIllegalPath(this.parseListUrl) +
      ".json";
    this.tableList = this.readJsonFile(this.jsonPath) || {};
    if (this.parseAllCategory) {
      this.currentPage = Object.keys(this.tableList).length || 1;
    }
    let parentDir =
